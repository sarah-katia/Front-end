...
def main():
    # Update the argument parser to accept either a file or JSON string
    parser = argparse.ArgumentParser(description='Batch update researcher publications')
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('--researchers-file', help='JSON file containing researchers data')
    group.add_argument('--researchers-json', help='JSON string with researchers data')
    ...
    parser.add_argument('--MAJyear', required=True, help='Updating year')
    args = parser.parse_args()
    
    
        # list of researchers file path
    try:
       ...
       # Load researchers data either from file or direct JSON string
        if args.researchers_file:
            with open(args.researchers_file, 'r') as f:
                noms_chercheurs = json.load(f)
        else:
            # Parse the JSON string directly
            noms_chercheurs = json.loads(args.researchers_json)
        ...
        with open(csv_file, mode='w', newline='', encoding='utf-8') as csv_file:
            ...
            for i, author_info in enumerate(noms_chercheurs,1):
                author_name = author_info["nom_complet"]
                author_id = author_info["chercheur_id"]
        ...
        with open(fichier_articles, mode="w", newline="", encoding="utf-8") as file:
            ...
            for chercheur in noms_chercheurs:
                ...
                id_chercheur = chercheur["chercheur_id"]  # Researcher ID
                        ...
                        if article_response.status_code == 200:
                            ...
                            for field in article_soup.find_all("div", class_="gs_scl"):
                                ...
                                if field_name == "Journal":
                                    ...
                                    if int(date_publication) < anneMAJ:
                                        continue
        ...
        # Start of the conference/journal classification code
        ...
        def search_qualis(conference_names, pub_id, input_file="qualis_ranks.csv", output_file=classment_output):
            ...
            # Assurez-vous que le fichier de classement existe et n'est pas vide
            attempts = 0
            while (not os.path.exists(input_file) or os.path.getsize(input_file) == 0) and attempts < 3:
                print(f"Warning: {input_file} is empty or missing. Attempting to download data...")
                extract_qualis_data()
                attempts += 1
            ...
            try:
                df = pd.read_csv(input_file)
                if df.empty and attempts < 3:
                    print(f"Warning: {input_file} contains no data. Re-downloading...")
                    extract_qualis_data()
                    df = pd.read_csv(input_file)  # Relire après téléchargement
            except (pd.errors.EmptyDataError, FileNotFoundError) as e:
                print(f"Error: {input_file} is unreadable or missing: {e}")
                if attempts < 3:
                    extract_qualis_data()
                    try:
                        df = pd.read_csv(input_file)
        ...
        os.makedirs(os.path.dirname(final_outpout), exist_ok=True)
        os.makedirs(os.path.dirname(classment_output), exist_ok=True)
        
        # Create files with headers if they don't exist
        if not os.path.exists(final_outpout):
            with open(final_outpout, mode='w', newline='', encoding='utf-8') as csv_file:
                writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
                writer.writeheader()
        
        if not os.path.exists(classment_output):
            with open(classment_output, mode='w', newline='', encoding='utf-8') as csv_file:
        ...
        # SCIMAGO section
        # URLs des classements
        SCIMAGO_URL = "https://www.scimagojr.com/journalrank.php?out=xls"
        HEADERS = {'User-Agent': 'Mozilla/5.0'}
        SCIMAGO_FILE = "scimago_downloaded_file.xls"
        MERGED_PUBLICATIONS = final_outpout
        OUTPUT_FILE = "ScimagoMerging.csv"
        RESULTS_FILE = classment_output  # Fichier des résultats filtrés

        # 📥 Fonction pour télécharger Scimago
        ...
        # 📊 Fusionner les publications avec Scimago
        def merge_publications_with_scimago(scimago_df):
            if not os.path.exists(MERGED_PUBLICATIONS):
                print(f"❌ Erreur : Le fichier {MERGED_PUBLICATIONS} est introuvable.")
                return None
        ...
        # Try to download and process Scimago data
        if download_scimago():
            scimago_df = load_scimago_data()
            merge_publications_with_scimago(scimago_df)
        else:
            print("Creating placeholder Scimago entries...")
            # Create placeholder entries for all publications
            try:
                publications_df = pd.read_csv(MERGED_PUBLICATIONS, encoding="utf-8", on_bad_lines='skip')
                processed_ids = set()
                results_list = []
                ...
                print(f"Failed to create placeholder Scimago entries: {e}")
        ...
        INPUT_FILE = final_outpout
        OUTPUT_FILE = classment_output
        PDF_A = "revues_A.pdf"
        PDF_B = "revues_B.pdf"

        # Download the PDFs with error handling
        ...
        # Download both PDFs
        a_success = download_pdf(URL_A, PDF_A)
        b_success = download_pdf(URL_B, PDF_B)

        # En-têtes pour éviter d'être bloqué par le site
        HEADERS = {'User-Agent': 'Mozilla/5.0'}
        ...
        def search_in_pdfs(journal_name, text_A, text_B):
            if not journal_name:
                return ""
        ...
        # Charger le texte des PDFs
        print("📥 Extraction du texte des fichiers PDF...")
        text_A = extract_text_from_pdf(PDF_A) if a_success else ""
        text_B = extract_text_from_pdf(PDF_B) if b_success else ""
        print("--------------------------------------------")
        ...
        with open(OUTPUT_FILE, mode="a", newline="", encoding="utf-8") as file:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    if journal_name:
                        classement = search_in_pdfs(journal_name, text_A, text_B)
        ...
        try:
            ...
            if os.path.exists(final_outpout):
                try:
                    df = pd.read_csv(final_outpout, encoding="utf-8", sep=None, engine="python", on_bad_lines='skip')
                    processed_ids = set()
                    
                    with open(classment_output, mode='a', newline='', encoding='utf-8') as csv_file:
                        writer = csv.writer(csv_file)
        ...
        with open("scraper_error.log", "a") as log_file:
